...
def cnt_label(dataset_dir_paths = ['../dataset_rz'], result_dir_path = '../results'):
    label_dir_path=os.path.join(result_dir_path,"cntlabel")
    ...
    for data_dir_path in dataset_dir_paths:
        # data_dir_path = '../dataset_rz'
        if os.path.isdir(data_dir_path):
            dir_name_list = np.sort(os.listdir(data_dir_path))
            # dir_name_list = ['019900', '019901', '019902', '019903', '019904', '019905', '019906', '019907", ...]
    ...
    with open(stat_file_path, 'w') as file_write_obj:
        ...
        file_write_obj.writelines(lines)
def cnt_label_zip(dataset_dir_paths = ['../dataset_rz'], result_dir_path = '../results'):
    label_dir_path=os.path.join(result_dir_path,"cntlabel")
    label_file_name = "cntlabel.txt"
    cnt_file_name = "cnt.txt"
    stat_file_name = "stat.txt"
    ...
    for data_dir_path in dataset_dir_paths:
        # data_dir_path = '../dataset_rz'
        if os.path.isdir(data_dir_path):
            zfile_name_list = np.sort(os.listdir(data_dir_path))
            # dir_name_list = ['019900.zip', '019901', '019902', '019903', '019904', '019905', '019906', '019907", ...]
            print("dir_name_list = {}".format(zfile_name_list))
            for ind, zfile_name in enumerate(zfile_name_list):
                zfile_path = os.path.abspath(os.path.join(data_dir_path, zfile_name))
                # print("dir_path[{}] = {}".format(ind,dir_path))
                if os.path.isdir(dir_path):
                    file_name_list = np.sort(os.listdir(dir_path))
                    num_of_files = len(file_name_list)
                    if num_of_files >= cnt_threshold :
                        file_num_dict[zfile_name] = num_of_files
                        cnt_label_dict[zfile_name] = (dir_path,True)
                        true_label_cnt = true_label_cnt + 1
                    elif num_of_files < cnt_threshold and num_of_files > cnt_threshold_valid:
                        file_num_dict[zfile_name] = num_of_files
                        cnt_label_dict[zfile_name] = (dir_path,False)
                        false_label_cnt = false_label_cnt + 1
                    else:
                        print("invalid cnt number={}".format(num_of_files))
